...
This tool allows you to manually create WhatsApp channels and fetch WhatsApp templates from Turn.io's WhatsApp Business API and insert them into your RapidPro database, bypassing the automatic cron job system.
...
1. **`create_whatsapp_channel`** - Creates a new WhatsApp channel in RapidPro
...
## Command 1: Create WhatsApp Channel

### Basic Syntax
```bash
poetry run python manage.py create_whatsapp_channel --org-id <ORG_ID> --number <PHONE_NUMBER> --verified-name <BUSINESS_NAME> --phone-number-id <PHONE_NUMBER_ID> --waba-id <WABA_ID> --business-id <BUSINESS_ID> [OPTIONS]
```
...
#### 2. Create Channel with Optional Fields (from container)
```bash
docker exec -e REMOTE_CONTAINERS=true -e POSTGIS=off -it rapidpro-rapidpro-1 poetry run python manage.py create_whatsapp_channel \
    --org-id 1 \
    --number "+1234567890" \
...
#### 3. Dry Run (Preview Only) - from container
```bash
docker exec -e REMOTE_CONTAINERS=true -e POSTGIS=off -it rapidpro-rapidpro-1 poetry run python manage.py create_whatsapp_channel \
    --org-id 1 \
    --number "+1234567890" \
...
### Step 1: Create the WhatsApp Channel
```bash
docker exec -e REMOTE_CONTAINERS=true -e POSTGIS=off -it rapidpro-rapidpro-1 poetry run python manage.py create_whatsapp_channel \
    --org-id 1 \
    --number "+1234567890" \
...
**Output:**
```
✅ Successfully created WhatsApp channel!
   Channel ID: 5
...
2. **Configuration**: Prepares channel configuration with all required fields
3. **Creation**: Uses RapidPro's existing `Channel.create()` method
4. **Output**: Returns channel ID for use with template sync
...
Channel Name: +1234567890 - My Business
Phone Number ID: 123456789
Channel Type: WhatsApp (WAC)
Configuration:
  wa_number: +1234567890
  wa_verified_name: My Business
  wa_waba_id: 987654321
  wa_currency: USD
  wa_business_id: 555666777
  wa_message_template_namespace: 
  wa_pin: ******
==================================================
```
...
- Use the query above to find WhatsApp channels
